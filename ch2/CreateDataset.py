...
class CreateDataset:
    ...
    def add_numerical_dataset(self, file, timestamp_col, value_cols, aggregation='avg', prefix=''):
        # print(f'Reading data from {self.base_dir}/{file} ...')
        ...
        for i in range(0, len(self.data_table.index)):
            ...
            relevant_rows = dataset[
                ...
                (dataset[timestamp_col] < (self.data_table.index[i] + timedelta(milliseconds=self.granularity)))
                ]
            for col in value_cols:
                ...
                if len(relevant_rows) > 0:
                    ...
                    elif aggregation == 'none' or aggregation is None:
                        if len(relevant_rows) > 1:
                            raise ValueError("More than 1 value found for 'none' aggregation")
                        self.data_table.loc[self.data_table.index[i], str(prefix) + str(col)] = relevant_rows[col].values[0]
                    else:

...
class VisualizeDataset:
    ...
    def save(self, plot_obj, formats=['pdf'], path_name: str = None):  # 'svg'
        if (path_name is not None) and (path_name != ''):
            fig_name = path_name
            self.figures_dir = Path('.')
        else:
            fig_name = f'figure_{datetime.strftime(datetime.now(), "%Y%m%d-%H%M%S")}'
    ...
    def plot_dataset(self, data_table, columns, match='like', display='line', show: bool = True, save: bool = True, path_name_wo_ext: str = None):
        ...
        for i in range(0, len(columns)):
            ...
            # We can match exact (i.e. a columns name is an exact name of a columns or 'like' for which we need to find columns names in the dataset that contain the name.
            if match[i] == 'exact':
            ...
            for j in range(0, len(relevant_cols)):
                ...
                if display[i] == 'points':
                    xar[i].plot(data_table.index[mask], data_table[relevant_cols[j]][mask], self.point_displays[j % len(self.point_displays)])
                else:
                    xar[i].plot(data_table.index[mask], data_table[relevant_cols[j]][mask], self.line_displays[j % len(self.line_displays)])

            xar[i].tick_params(axis='y', labelsize=10)
            xar[i].legend(relevant_cols, fontsize='xx-small', numpoints=1, loc='upper center', bbox_to_anchor=(0.5, 1.3), ncol=len(relevant_cols), fancybox=True, shadow=True)
        ...
        plt.xlabel('time')
        if save:
            self.save(plt, path_name=path_name_wo_ext)
        if show:
            plt.show()

    def plot_xy(self, x, y, method='plot', xlabel=None, ylabel=None, xlim=None, ylim=None, names=None, line_styles=None, loc=None, title=None):
        for input in x, y:
